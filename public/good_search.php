...
$postId = $instagram->getByGood($login_user['id']);

$post = [];

foreach($postId as $num){
	$post[] = $instagram->getById($num['post_id']);
}
...
<body>
<section id="good_searchphp">

<h2 class="search-word">検索カテゴリー：いいね</h2>

<?php if(!$post){echo $instagram->h($login_user['username']).'さんのいいねが含まれた投稿はありません。';}?>
...
<?php if(empty($post_all)):?>
	...
	<p><?php echo $instagram->h($login_user['username']).'さんのいいねが含まれた投稿はありません。'?></p>
<?php endif;?>
